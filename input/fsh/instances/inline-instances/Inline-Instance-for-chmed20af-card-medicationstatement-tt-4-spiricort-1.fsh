Instance: Inline-Instance-for-chmed20af-card-medicationstatement-tt-4-spiricort-1
InstanceOf: Medication
Usage: #inline
* id = "cont1"
* code = urn:oid:2.51.1.1#7680388400291 "SPIRICORT Filmtabl 20 mg"
* code.text = "SPIRICORT Filmtabl 20 mg"
* form = urn:oid:0.4.0.127.0.16.1.1.2.1#10221000 "Film-coated tablet"
* ingredient.itemCodeableConcept = $sct#116601002 "Prednisolone (substance)"
* ingredient.itemCodeableConcept.text = "Prednisolone"
* ingredient.strength.numerator = 20 'mg' "milligram"
* ingredient.strength.denominator = 1 http://snomed.info/sct#732936001 "Tablet (unit of presentation)"